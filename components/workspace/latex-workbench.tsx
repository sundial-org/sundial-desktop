...
export function LatexWorkbench({ editor, preview, isMobile, viewMode, onViewModeChange }: LatexWorkbenchProps) {
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      setEditorWidth((current) => {
        ...
        if (current !== null && current >= nextWidth - DIVIDER_WIDTH - 1) {
          return Math.max(0, nextWidth - DIVIDER_WIDTH);
        }
        const fallback = nextWidth * (storedFraction ?? DEFAULT_EDITOR_FRACTION);
        return clampLeftWidth(current ?? fallback, nextWidth);
      });
    };
  ...
  }, [isMobile, storedFraction]);

  useEffect(() => {
    if (isMobile || typeof window === 'undefined' || containerWidth <= 0 || editorWidth === null) return;
    if (editorWidth <= 0 || editorWidth >= containerWidth - DIVIDER_WIDTH) return;
    window.localStorage.setItem(STORAGE_KEY, String(editorWidth / containerWidth));
  }, [containerWidth, editorWidth, isMobile]);
  ...
  const resolvedEditorWidth = useMemo(() => {
    ...
    if (editorWidth === null) {
      return clampLeftWidth(containerWidth * (storedFraction ?? DEFAULT_EDITOR_FRACTION), containerWidth);
    }
    return editorWidth;
  }, [containerWidth, editorWidth, storedFraction]);

  const collapseState: 'none' | 'editor' | 'preview' = useMemo(() => {
    if (resolvedEditorWidth === null || containerWidth <= 0) return 'none';
    if (resolvedEditorWidth <= 0) return 'editor';
    if (resolvedEditorWidth >= containerWidth - DIVIDER_WIDTH) return 'preview';
    return 'none';
  }, [resolvedEditorWidth, containerWidth]);
  ...
  useEffect(() => {
    if (viewMode === undefined || isMobile || containerWidth <= 0) return;
    if (lastAppliedViewModeRef.current === viewMode && editorWidth !== null) return;
    lastAppliedViewModeRef.current = viewMode;
    setEditorWidth(viewModeToEditorWidth(viewMode, containerWidth, storedFraction));
  }, [viewMode, containerWidth, storedFraction, isMobile, editorWidth]);
  ...
  useEffect(() => {
    ...
    // Only react to collapseState; including viewMode would race the apply effect.
  ...
  const handlePointerUp = useCallback((event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    if (!dragState.moved) {
      const cw = dragState.containerWidth;
  ...
  const handleKeyDown = useCallback((event: KeyboardEvent<HTMLDivElement>) => {
    ...
    setEditorWidth((current) =>
      clampLeftWidth(
        (current ?? containerWidth * (storedFraction ?? DEFAULT_EDITOR_FRACTION)) + delta,
        containerWidth,
      ),
    );
  }, [containerWidth, storedFraction]);
  ...
  const { minLeft, maxLeft } =
    containerWidth > 0 ? getPaneBounds(containerWidth) : { minLeft: 0, maxLeft: 100 };
  const separatorValue =
    containerWidth > 0 && resolvedEditorWidth !== null
      ? Math.round((resolvedEditorWidth / containerWidth) * 100)
  ...
  return (
    ...
    >
      ...
      <div
        ...
        aria-valuemin={Math.round((minLeft / Math.max(containerWidth, 1)) * 100)}
        aria-valuemax={Math.round((maxLeft / Math.max(containerWidth, 1)) * 100)}
        ...
        className={`group relative flex shrink-0 touch-none cursor-col-resize flex-col items-center justify-center gap-1 bg-stone-50 outline-none transition-colors ${
