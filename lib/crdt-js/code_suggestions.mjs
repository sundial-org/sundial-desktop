...
// Every live suggestion id in the ledger. Reads only an existing root — getMap
// on a missing one would CREATE it, mutating the doc from a read path.
export function liveCodeSuggestionIds(doc) {
  if (!doc.share.has(CODE_SUGGESTIONS_ROOT)) return [];
  const ids = new Set();
  const map = suggestionsMap(doc);
  for (const key of map.keys()) {
    const entryId = map.get(key)?.get?.('id');
    if (typeof entryId === 'string' && entryId) {
      ids.add(entryId);
      continue;
    }
    const hash = key.indexOf('#');
    ids.add(hash === -1 ? key : key.slice(0, hash));
  }
  return [...ids];
}
