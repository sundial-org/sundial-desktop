...
import { applyCodeSuggestion, hasCodeSuggestion, codeSuggestionResolution, liveCodeSuggestionIds } from './code_suggestions.mjs';
import {
  Y,
  applyMarkdownDiff,
...
// Every live suggestion id in the doc — inline/structural marks for markdown,
// ledger entries for code. The enumeration twin of hasSuggestionMark (kept in
// exact attribute correspondence with it), for callers that need the full
// pending set rather than one probe.
export function collectLiveSuggestionIds(documentName, document) {
  if (!isMarkdownDocument(documentName)) return liveCodeSuggestionIds(document);
  const ids = new Set();
  scanSuggestionMarks(
    document,
    (a) => {
      if (a.insertion?.id) ids.add(a.insertion.id);
      if (a.deletion?.id) ids.add(a.deletion.id);
      if (a.modification?.id) ids.add(a.modification.id);
      if (a.suggestionInsertionId != null) ids.add(a.suggestionInsertionId);
      if (a.suggestionDeletionId != null) ids.add(a.suggestionDeletionId);
      if (Array.isArray(a.suggestionModifications)) {
        for (const mod of a.suggestionModifications) if (mod?.id) ids.add(mod.id);
      }
      return false; // never "found" — walk the whole doc, collecting
    },
    { includeElementAttributes: true },
  );
  return [...ids];
...
export function applySuggestionIfChanged(documentName, document, contentText, suggestionId, meta = {}) {
  // Idempotent against the poll / cold-load re-seeing a suggest row.
  if (hasSuggestionForId(documentName, document, suggestionId)) return false;
  const oldText = readDocumentText(documentName, document);
  return isMarkdownDocument(documentName)
    ? applyMarkdownSuggestion(document, oldText, contentText, suggestionId)
    : applyCodeSuggestion(document, oldText, contentText, suggestionId, meta);
}

/**
 * Is this suggestion id already spoken for in the doc — staged as a live
 * entry/marks, OR already resolved (tombstone)? Without the tombstone half, a
 * re-applied row would re-stage a suggestion the user already accepted or
 * rejected. This is the sole gate `applySuggestionIfChanged` skips on, exported
 * so callers can tell an id collision apart from its other `false` returns
 * (notably a markdown change the codec cannot represent as marks).
 */
export function hasSuggestionForId(documentName, document, suggestionId) {
  return isMarkdownDocument(documentName)
    ? hasSuggestionMark(document, suggestionId) || Boolean(markdownSuggestionResolution(document, suggestionId))
    : hasCodeSuggestion(document, suggestionId) || Boolean(codeSuggestionResolution(document, suggestionId));
}
