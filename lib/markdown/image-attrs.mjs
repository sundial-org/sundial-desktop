...
/**
 * Obsidian sizing spec in an image's ALT text: `![alt|640x480](src)` /
 * `![alt|100](src)`. Render-layer only — the alt (and so the file's bytes)
 * stays verbatim; callers apply the returned dimensions to the rendered <img>.
 * An explicit `width` attr (Sundial's own `{width=N}` syntax or a manual
 * resize) wins over the alt spec.
 */
export function parseAltSizeSpec(alt) {
  const m = String(alt ?? '').match(/\|(\d{1,5})(?:x(\d{1,5}))?$/);
  if (!m) return null;
