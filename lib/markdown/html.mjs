...
import { imageMarkdown } from './image-attrs.mjs';
...
function renderInline(inline, options) {
  ...
  for (const node of inline) {
    ...
    if (node.type === 'image') {
      ...
      const width = Number(node.width) > 0 ? ` width="${Math.round(Number(node.width))}"` : '';
...
function renderBlock(block, options) {
  switch (block.type) {
    ...
    case 'horizontalRule':
      return '<hr>';
    case 'codeBlock':
      return `<pre><code class="language-${escAttr(block.language)}">${esc(block.text)}</code></pre>`;
    case 'blockquote':
...
function renderTable(block, options) {
  ...
  const row = (tag, cells) => `<tr>${pad(cells)
    .map((cell) => `<${tag}><p>${renderInline(cell, options)}</p></${tag}>`)
    .join('')}</tr>`;
