...
// 3+ backticks (CommonMark); the info string may not contain a backtick, so a
// line like "``` `x` ```" stays inline code, not a fence. The fence length is
// captured: the CLOSING fence must be at least as long, which is what lets a
// ````-fenced block carry ``` lines as content instead of ending early.
const MD_CODE_FENCE = /^(`{3,})([^`]*)$/;
...
function parseBlockLines(lines, topLevel = false) {
  ...
  while (i < lines.length) {
    ...
    if (fence) {
      ...
      const codeLines = [];
      i += 1;
      // CommonMark closing fence: >= as many backticks as the opening, up to
      // 3 spaces of indent, nothing else. A shorter run stays content — that
      // is what keeps a ```-carrying block inside a ```` fence intact.
      const closes = (l) => {
        const m = l.match(/^ {0,3}(`{3,})\s*$/);
        return Boolean(m && m[1].length >= openLen);
      };
